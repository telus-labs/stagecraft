...
| `devteam prototype <start\|note\|promote> [id-or-title] [--feature "..."] [--feedback "..."] [--track <t>] [--json]` | **Pre-SDLC prototype mode.** `start` creates `pipeline/prototypes/<id>/` with `intent.md`, `build-prompt.md`, `feedback.md`, `promotion.md`, and `prototype.json`. `note` appends demo/stakeholder feedback. `promote` updates the handoff with `devteam run --feature-file pipeline/prototypes/<id>/promotion.md --track <t>`. Prototype packets are learning records, not gate evidence, and cannot satisfy sign-off/deploy. |
