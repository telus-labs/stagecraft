...
| **Role** | `roles/<role>.md` | The framework + your customizations | A named seat at the team — `pm`, `principal`, `backend`, `frontend`, `platform`, `qa`, optional `documentation`, `reviewer`, `security`. A role's brief is the source of truth for what it does, reads, and writes. |
| **Workstream** | derived at dispatch time | The orchestrator | One dispatch of a stage to one effective role. Single-role stages have one workstream; multi-role stages (build, peer-review) have several. Optional roles are gate-selected and never join the default matrix. **The workstream is the unit of gate identity.** Stage 5 (peer-review) is a special case — see the §stage-05 vocabulary callout below. |
| **Host** | `hosts/<host>/` (first-party) or a `@devteam/host-<name>` plugin package | You choose at `devteam init` | The AI tool that actually runs the model: `claude-code`, `codex`, `antigravity`, `omp`, `omnigent`, `openai-compat`, `acp`, or `generic` (no host) ship first-party; `gemini-cli` is a plugin (`packages/host-gemini-cli/`, phase 34.4 — deprecated upstream). |
...
| **Track** | `core/pipeline/stages.js` | Your `.devteam/config.yml` (`pipeline.default_track`) | Which stages run for this kind of change. Ten tracks: `full`, `quick`, `nano`, `config-only`, `dep-update`, `hotfix`, `loop`, `review-only`, `review-pr`, `refactor`. Tracks shape *which* stages run; never *what* a stage does. |
...
- **Capability** — a flag in `hosts/<host>/capabilities.json` declaring what the host supports: `hooks`, `subagents`, `slashCommands`, host-native `worktrees`, `headless`, plus an `enforces` map saying where each core rule is enforced (`tool-call-time`, `post-hoc-audit`, `prompt-only`). Core-managed `pipeline.workstream_isolation: git-worktree` is host-neutral and does not depend on that adapter capability. `enforces.tool_budget: "native"` means the host enforces the role's declared tool list at the tool-call boundary (claude-code subagent `tools:` line); `"prompt-only"` means the budget is injected as an advisory prompt instruction only.
- **Tool budget** — the host-neutral per-role set of permitted tools, stored in `core/roles.js`. The orchestrator resolves it before host routing and carries it as `descriptor.toolBudget` through the dispatch pipeline; adapters enforce or render it according to their capabilities. Claude Code's `ROLE_FRONTMATTER` contains only host-specific subagent metadata. The budget is stamped on workstream gates as `dispatched_tool_budget` for audit legibility alongside the existing `tools_hash` (C4 reproducibility).
...
- **Stage descriptor** — what the orchestrator hands to an adapter to render a prompt: stage id, role, workstream id, objective, files-to-read, allowed-writes, exact approved affected files (when selected), artifact path, template name, gate skeleton.
...
- **Security heuristic** — file-path patterns plus security-sensitive added lines (`bcrypt`, JWT, crypto APIs, etc.) that set `security_review_required: true` in pre-review's gate. Scanning added lines avoids re-triggering on unrelated edits to files with historical examples. That value triggers the conditional `stage-04b` security review.
...
- **Migration safety (stage-04d)** — conditional safety review for data-layer diffs. Fires when stage-04a's heuristic flags schema files, migration directories, or DDL in added lines. Walks six questions (what does it do? breaking? backfill required? dual-write required? rollback plan? rollback tested?) and writes a gate covering each. **Has veto power**: an empty rollback plan, an untested rollback on a breaking change, or a missing backfill strategy on a backfill-required migration each auto-veto. Peer-review approvals CANNOT override a veto — the migrations role must personally re-review the fix. See `skills/migration-safety/SKILL.md` for methodology and the auto-veto criteria.
- **Verification beyond tests (stage-06d)** — full-track-only stage that runs AFTER stage-06 (qa) PASS. New `verifier` role applies property-based testing (fast-check / hypothesis / proptest), mutation testing (stryker / mutmut / mull), and/or formal verification (TLA+ / Alloy / Lean) to the changed code. Read-only on production code; writes property tests under `src/tests/property/` and formal specs under `pipeline/formal/`. Gate carries `methods_attempted[]`, `methods_skipped[{method, reason}]`, per-method stats, and `blocking_findings[]` (a counterexample to a stated invariant, a surviving mutant on a critical path, or a formal counterexample to a safety property). Non-empty `blocking_findings` halts sign-off; the implementer addresses, then the stage re-runs. The role brief prohibits fabricating results: if a tool isn't installed, the method is `attempted_but_blocked` with the install hint, not a fabricated score. Since phase-35 item 35.3, `methods_attempted[]` is no longer trusted on the verifier's say-so: `core/verify/stamp.js#stampStage06d` re-runs each claimed method (reusing the 31.4 mutation runner, detecting property-test tooling, checking formal-tool exit codes) and downgrades a claim with no executable evidence to `attempted_but_blocked:<method>`. See `skills/verification-beyond-tests/SKILL.md` for the five-phase procedure and the property-shape vocabulary (round-trip / idempotence / commutativity / monotonicity / invariant preservation / oracle).
- **Persistent memory** — `devteam memory ingest|query` builds a per-project retrieval index of briefs, design specs, ADRs, retros, and lessons. Dependency-free lexical retrieval is the default; a transformer semantic provider is opt-in.
- **Project Knowledge Pack** — the bounded learned-context block every stage receives: deterministic repository facts, reviewed patterns with outcome evidence, and attributed semantic-memory hits. See [`project-knowledge.md`](project-knowledge.md).

---

## Tracks at a glance

Ten tracks control which stages run: `full`, `quick`, `nano`, `config-only`, `dep-update`, `hotfix`, `loop`, `review-only`, `review-pr`, `refactor`. Each is a subset of the full 18-stage pipeline tuned to a change size and risk level. `full` runs everything; `nano` runs just build + a scoped peer-review + qa; `loop` is the smallest build-shaped track — one build workstream + qa + one reviewer, no design or deploy; `review-only` runs no build at all — security-review + red-team + peer-review of code that already exists; `review-pr` is the internal single-stage track `devteam review-pr` dispatches against — a scoped peer-review of one materialized PR, not meant to be picked directly via `--track`; `refactor` is `nano`'s build/peer-review/qa shape with the bar shifted to behavior preservation — a characterization brief before any structural change, no new acceptance criteria, and the 31.4 mutation smoke gate on by default (phase-35 item 35.5).
...
| `.devteam/knowledge/` (in target project) | Fingerprinted static project facts injected automatically; operational and gitignored. |
