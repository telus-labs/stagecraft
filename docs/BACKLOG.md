...
| B8 | **Cross-artifact consistency analyze** `[cmp-E-1]` | 4 | 2 | New `devteam consistency analyze` walks brief → design-spec → spec.feature → pr-\*.md → test-report → red-team-report → gates and reports drift in one pass. Generalizes G2's brief↔spec↔test drift check across all intermediate artifacts. From Spec Kit's `/speckit.analyze`. Catches a class of bugs gates alone miss (each gate passes but the chain is incoherent). Builds on existing `core/spec/verify.js`. |
...
1. **B8 — Cross-artifact consistency analyze** `[cmp-E-1]` (4 / 2). Smallest effort; sits on top of G2's already-shipped foundation; catches a class of bugs the existing gate machinery misses; produces a sub-deliverable that feeds the future complexity-assessment stage. **Start here.**
2. **C7 — `eslint-plugin-security`** `[hist-a]` (3 / 1). Trivial effort, empirically justified by 3 CodeQL alerts in one week. Validates the "add it next time" promise from PR 2.1.
