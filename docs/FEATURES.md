...
- **Run plan preview and right-sizing (Phase 26 / #316).** Before dispatch, non-JSON `devteam run` prints the effective track/custom plan, included/total stage count, configured skip count, deterministic right-sized skip count, conditional-stage count, base workstream count, expected workstream count, and candidate active roles. The same typed `run-plan` event is written to `run-log.jsonl` so tools can answer "how big is this run?" without parsing terminal prose.
