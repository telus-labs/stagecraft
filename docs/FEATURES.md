...
and never transmits them. After a human reviews a successful fix/retry,
`devteam evidence accept-resolution --yes` records one hash-bound, privacy-bounded
acceptance for H3 readiness; it never stores resolution text or learns a recipe. See
[Evidence Readiness](evidence.md).
...
- Priority order: hotfix keywords → dep-update → config-only → nano → quick → explicit loop language → loop (day-to-day fallback)
- Heuristic overrides: migration-safety-required bumps lighter tracks to full; security-required code work on `loop`, `nano`, or `quick` bumps to full because those tracks do not contain security-review
...
- Includes the durable `pipeline/run-plan.json` when present, while continuing to exclude volatile lock/state/log files
...
**`devteam run [--feature "..."] [--repair "<symptom>"] [--repair-at <file>:<line>] [--track <t>] [--until <s>] [--budget-usd X] [--budget-tokens N] [--timeout-ms N] [--retry-delay-ms N] [--auto-rule <classes>] [--allow-stage <s>] [--max-iterations N] [--resume] [--force] [--json] [--auto-commit] [--watch]`** — drive the whole pipeline unattended. See the **Autonomous pipeline execution** section under Advanced AI capabilities (and [`docs/runbooks/autonomous-run.md`](runbooks/autonomous-run.md)) for the full behavior.
...
- Writes a durable `pipeline/run-plan.json`, plus volatile `pipeline/run.lock`, `run-state.json`, and the audit-trail `run-log.jsonl`
...
- **Usage caps** — `--budget-usd` halts on recorded spend and `--budget-tokens` halts on trusted observed/estimated token usage. Token accounting is cumulative across retries and resumes, ignores model-authored claims, and reports basis/coverage in run, status, and HTML outputs. Both caps are checked before dispatch and can overshoot by one dispatch. `--resume` inherits omitted caps and rejects conflicting cap or track values before dispatch. With neither cap, a two-line advisory is written to stderr; behavior and exit codes are otherwise unchanged.

**Dogfooding guide — Phase 14.5. Phase 14 complete.** [`docs/guides/dogfooding.md`](guides/dogfooding.md) covers the complete workflow for running Stagecraft against its own source tree: prerequisites, one-time setup with `devteam init --host claude-code --profile dogfood`, per-feature workflow (branch → bounded `devteam run` → review → commit → clean up), recommended budget table by pipeline depth, explanation of the pre-commit infrastructure guard, failure-mode table, and expected `devteam doctor` output for the "Dogfood mode" section.
...
- Gate-watch resource errors degrade live refresh with a warning instead of crashing the HTTP server; API and static views remain available
...
- **Auto-fixes machine-diagnosable failures.** On a `code-defect` it clears the failing gate, writes the blockers into `pipeline/context.md` as cross-stage context, and re-dispatches — bounded by `autonomy.max_retries` (default 2). When blockers name target paths or a requested artifact, a pre-clear ownership check compares every target with candidate build roles' existing `roleWrites`; it deterministically selects a compatible owner or halts as `retry-ownership` before clearing live gates or invoking a host. It never widens a role's write surface. Transient dispatch failures (no gate written) back off and retry; structural ones (clean exit, no output) halt. Retry events include `retry_reason` and `backoff_class`; immediate non-timeout no-gate failures use short backoff while timeouts keep the full delay.
...
- **Safety rails.** An exclusive `pipeline/run.lock`, resumable `run-state.json` (`--resume`), a materialized `pipeline/run-plan.json`, pre-dispatch dollar and provider-neutral token caps, a per-stage `--timeout-ms`, and a `--max-iterations` guard. The run plan records resolved track provenance, the ordered preflight stage-disposition snapshot, candidate role/host/model routes, an execution fingerprint, and the fingerprint-bound effective caps/scoped stoplist ruling. Resumes preserve the original caps and reject conflicting caps or track; an unchanged hashed feature/brief/policy reuses its audited stoplist bypass. Dynamic right-sizing and upstream-gate conditions remain runtime decisions.
- **`--auto-commit` — unattended commit after a clean halt (Phase 12.3, ADR-010).** When passed, the driver automatically commits pipeline artifacts after a clean halt (`ceiling`, `--until`, `budget`) using the same algorithm as `devteam commit` — no interactive prompt. Non-clean halts (`fix-and-retry`, `resolve-escalation`, `structural-input`, etc.) never trigger auto-commit. On commit failure the run's exit code is unchanged; a warning is emitted to stderr and `auto-commit-failed` is logged to `run-log.jsonl`. Nothing to commit → `auto-commit-skipped` logged and the halt proceeds normally.
- **Materialized run plan and right-sizing (ADR-018 / #316).** Before dispatch, non-JSON `devteam run` prints the effective track/custom plan, included/total stage count, configured skip count, deterministic right-sized skip count, conditional-stage count, track-sized base workstream count, expected workstream count, candidate active roles, and the plan path/fingerprint. The full plan is written atomically to `pipeline/run-plan.json`; the same typed `run-plan` event is appended to `run-log.jsonl` so tools can inspect it without parsing terminal prose. Configured selection is resume-bound. Right-sized skips carry `decision_basis: "preflight-snapshot"` and `reevaluated_at_runtime: true`; upstream-gate decisions are honestly labelled `conditional`; routes are `candidate` until runtime discovery confirms them.
- **Deterministic safe skips.** `devteam next` skips clarification, accessibility, observability, advanced verification, and performance gates only when changed files and brief/design context lack the relevant trigger. `devteam run` records each skip as a typed `skip-stage` event with trigger inputs. `pipeline.force_stages` overrides a skip for a named stage, and `pipeline.right_sizing: false` disables the deterministic rules globally.
- **Audit trail.** Every transition — including each auto-fix, deterministic skip, and auto-ruling with its `grant_class` and authority — is appended to `pipeline/run-log.jsonl`. Autonomous dispatches also append a privacy-bounded `dispatch-observation` per non-skipped workstream (stage, role, host, model, status, gate-written/timeout flags, and optional numeric cost/duration), giving `devteam evidence` durable routing history without copying free-form gate text. Explicit accepted-resolution events bind a human decision to a successful retry using only typed categories and hashes.
- **Per-host workstream scheduling (Phase 26 / #317).** `routing.host_concurrency` caps simultaneous workstreams per host inside an already-ready stage. Stage order, gate filenames, restart behavior, and consequence ceilings do not change. `run-log.jsonl` records `workstream-queued` plus `queue_ms` on start/finish/dispatched events so overloaded local hosts and provider queues are visible.
...
- **Critical-path report (Phase 26 / #313).** `devteam performance critical-path` reads `run-log.jsonl` plus orchestrator-stamped gates and reports dispatch wall time, queue wait, merge time, retry delay, summed workstream compute, estimated parallel savings, telemetry coverage, and repeated verification-command candidates. JSON output is available for p50/p95 baselines across real projects.
- **Stage DAG waves (ADR-017, phase-32 item 32.6).** Two curated `dependsOn` pairs/groups in `core/pipeline/stages.js` — `{pre-review ∥ red-team}` (both ready once `build` PASSes) and `{accessibility-audit ∥ observability-gate ∥ verification-beyond-tests ∥ performance-budget}` (all four ready once `qa` PASSes) — let the driver dispatch multiple ready stages concurrently instead of strictly in declared order. `autonomy.max_parallel_stages` (default **2**; `1` = off, byte-identical to pre-ADR-017 dispatch) caps wave size in `.devteam/config.yml`. `run-log.jsonl` gains a `wave_id` (shared across a real wave's member events, monotonic per run, absent entirely on a size-one wave) and a `wave-formed` event; `state.iterations`/`--max-iterations` count one wave as one iteration. Only fresh/resuming dispatches (`run-stage`/`continue-stage`) are batched — a `fix-and-retry` ready-set always collapses to a single dispatch. `devteam performance critical-path` gains a `waves` array and `wave_realized_savings_ms` (`sum(member durations) − max(member durations)`, additive to the existing sequential accounting, not a correction to it).
...
### Production feedback contract — close the brief→production SLO loop (G3)

After deploy, the operator copies `templates/production-feedback-template.md` to `pipeline/production-feedback.md` and fills in production signals — SLO/metric deltas vs. the brief's targets, incidents since deploy, adoption signals. No automated ingestion; the file is the integration point (Jira/Datadog automation can write it without framework changes).
...
- Effort-1 by design: no automation, no new commands, no integrations. The file is the contract; automation plugs in later at the write side with no framework changes
