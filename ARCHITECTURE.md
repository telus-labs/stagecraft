...
2. **The core never spawns a model.** It emits prompts and validates outputs; host adapters decide how the model consumes a prompt. Pipeline stages call `adapter.invoke()` through the orchestrator. One-off workflows such as rulings, accessibility remediation, reflection, and read-only `devteam chat` call the same adapter boundary through `core/adapters/invoke-task.js`; core modules must not parse `headlessCommand` or spawn a host CLI themselves.
3. **Gate JSON is the stable contract.** Versioned, validated, host-agnostic. Identity fields: `stage`, `workstream` (per-workstream only), `orchestrator`, `host` (per-workstream only), `status`. Multi-role stages produce per-workstream gates that the orchestrator merges into a stage-level gate with a `workstreams: []` array. The merge also preserves stage-specific fields consumed downstream: Stage 7 takes product/docs authorization from PM and runbook readiness from platform, so Stage 8 never infers deploy authority from aggregate status alone. No host-specific fields in the schema — those go in the adapter. Stage-level `chain` metadata may include the provider-neutral HMAC fields defined by [ADR-011](docs/adr/011-authenticated-gate-chain.md); secrets and KMS-provider details never enter the gate schema.
...
6. **Two independent isolation axes ([ADR-019](docs/adr/019-isolated-build-workstreams.md)).** `pipeline.isolation` chooses run-artifact placement (`in-place` or feature-bounded); `pipeline.workstream_isolation` chooses whether parallel stage-04 coding roles share the checkout or receive core-managed detached Git worktrees. The core reconciles only role-authorized changes and fails unresolved overlap; adapters continue to own model invocation.
...
12. **Bounded autonomous execution ([ADR-003](docs/adr/003-bounded-autonomous-execution.md), [ADR-018](docs/adr/018-materialized-run-plan-and-loop-default.md)).** The pipeline may be driven by a deterministic code loop (`devteam run`, `core/driver.js`) built on a typed failure model. Non-pass outcomes are classified by *required response*: the **gate-time** classes (`state-corruption`, `judgment-gate`, `external-blocked`, `code-defect`, plus `convergence-exhausted`) are carried as `failure_class` on `next()` action objects; the **dispatch-time** classes (`transient`, `structural-input`) are derived by the driver (`classifyDispatch`) from the `runHeadless` return, since only it holds that signal — a no-gate dispatch is retried with backoff, then halts as structural. A structured fix/retry target is dispatched only after its paths are proven compatible with one of the candidate build roles' existing `roleWrites`; no compatible owner halts as `retry-ownership` before gates are cleared or a host is invoked. Before dispatch, the driver materializes `pipeline/run-plan.json`: resolved track/provenance, ordered stage dispositions, candidate role/host/model routes, an execution fingerprint, and a separately bound safety policy containing the effective usage caps and any scoped stoplist bypass. Deterministic skips are explicit; unresolved gate-dependent stages are labelled conditional. `--resume` reuses the original plan only when its execution fingerprint and effective caps still match, preventing silent config/routing drift or an accidentally uncapped continuation. A stoplist bypass is valid only for its hashed feature, brief, and stoplist policy and is re-authorized explicitly when any of those changes. The plan is an inspectable execution contract, not gate evidence. Autonomy is bounded by a **consequence ceiling**: the driver never advances into stage-07 sign-off / stage-08 deploy without an explicit human grant (`--allow-stage`). Escalations halt for a human by default; an opt-in, CLI-only, allowlist-only `--auto-rule <classes>` lets the driver auto-apply Principal rulings whose `[class:]` is granted — never a typed `PRINCIPAL-CANNOT-DECIDE` (authority/information/value), the ceiling, or `convergence-exhausted` — recording authority provenance to `run-log.jsonl`. The driver introduces run-scoped state the stateless-within-a-run model otherwise avoids — `pipeline/run.lock`, `run-state.json`, `run-log.jsonl` — while `run-plan.json` is intentionally durable and reviewable. This refines decision #8: the driver *may gate dispatch* on tracked cost (a pre-dispatch `--budget-usd` check), while API-level budget enforcement still belongs to the host. The human role shifts from mechanical sequencer to authority grantor. Built in phases (failure model → driver skeleton → autonomous fix-and-retry → recipe factory); only the failure model is foundational and unconditional.
13. **Execution trust is explicit and fail-closed ([ADR-020](docs/adr/020-execution-trust-profiles.md)).** `trusted` host CLIs run as the operator and are not OS-sandboxed. `contained` wraps adapter-owned invocation in a non-root, resource-bounded Docker process and reconciles only allowed outputs from a disposable ADR-019 worktree. `remote` is reserved until a transport contract exists. The selected boundary is materialized and fingerprinted in the run plan; an unavailable requested boundary is an error, never an implicit downgrade.
14. **Memory has a useful zero-dependency floor ([ADR-021](docs/adr/021-dependency-free-memory-default.md)).** The default retriever is builtin signed feature hashing: offline, deterministic, and free of native/transitive packages. Neural transformer retrieval is an explicit operator opt-in because it expands install cost, licensing, and advisory scope. Embedder model IDs remain bound to stored vectors, so changing providers requires reindexing rather than silently mixing incompatible representations.
15. **Optional workstreams require explicit machine authority ([ADR-022](docs/adr/022-exact-file-documentation-workstream.md)).** `roles` remains the default stage matrix; `optionalRoles` may add a workstream only through its documented gate-selection contract. The first such role, `documentation`, requires a PASS Stage 1 gate with `active_roles: ["documentation"]` and a non-empty exact documentation `affected_files` list. That list is carried unchanged to build, QA, peer review, isolation reconciliation, and retries. No adapter, role, or retry may convert it into a directory or glob authority.
